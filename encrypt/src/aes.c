#include <stdio.h>
#include "aes.h"
#include "key.h"

// ---------------------HELPERS--------------------------------------------------------------------
const uint8_t sbox[256] = {
    
};

static uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0; // product
    for (int i = 0; i < 8; i++) {
        if (b & 1)
            p ^= a; // add a to p if LSB of b is 1

        uint8_t hi_bit_set = a & 0x80; // check if x^7 term is set
        a <<= 1; // multiply by x

        if (hi_bit_set)
            a ^= 0x1B;// modulo reduction with m(x)

        b >>= 1; // next bit of b
    }
    return p;
}

void shift_row_n(uint8_t *row, uint8_t n) {
    uint8_t temp;
    switch (n) {
        case 1:
            temp = row[0];
            row[0] = row[1];
            row[1] = row[2];
            row[2] = row[3];
            row[3] = temp;
            break;
        case 2:
            temp = row[0];
            row[0] = row[2];
            row[2] = temp;
            temp = row[1];
            row[1] = row[3];
            row[3] = temp;
            break;
        case 3:
            temp = row[3];
            row[3] = row[2];
            row[2] = row[1];
            row[1] = row[0];
            row[0] = temp;
            break;
        default:
            break;
    }
}
// ------------------------------------------------------------------------------------------------

void plaintext_to_state(const uint8_t *plaintext, state_t *state) {
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            (*state)[i][j] = plaintext[i + 4 * j];
        }
    }
}

void state_to_ciphertext(const state_t *state, uint8_t *ciphertext) {
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            ciphertext[i + 4 * j] = (*state)[i][j];
        }
    }
}

void sub_bytes(state_t *state) {
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint8_t j = 0; j < 4; ++j) {
            (*state)[i][j] = sbox[(*state)[i][j]];
        }
    }
}

void shift_rows(state_t *state) {
    shift_row_n((*state)[1], 1);
    shift_row_n((*state)[2], 2);
    shift_row_n((*state)[3], 3);
}

void mix_columns(state_t *state) {
    uint8_t temp[4];
    for (uint8_t c = 0; c < 4; c++) {
        for (uint8_t r = 0; r < 4; r++) {
            temp[r] = (*state)[r][c];
        }
        (*state)[0][c] = gf_mul(0x02, temp[0])  ^ gf_mul(0x03, temp[1]) ^ temp[2]               ^ temp[3];
        (*state)[1][c] = temp[0]                ^ gf_mul(0x02, temp[1]) ^ gf_mul(0x03, temp[2]) ^ temp[3];
        (*state)[2][c] = temp[0]                ^ temp[1]               ^ gf_mul(0x02, temp[2]) ^ gf_mul(0x03, temp[3]);
        (*state)[3][c] = gf_mul(0x03, temp[0])  ^ temp[1]               ^ temp[2]               ^ gf_mul(0x02, temp[3]);
    }
}

void add_round_key(state_t *state, const word* words, const int round) {
    uint8_t temp[4];
    for (uint8_t c = 0; c < 4; c++) {
        for (uint8_t r = 0; r < 4; r++) {
            temp[r] = (*state)[r][c];
        }
        (*state)[0][c] = temp[0] ^ words[round+c][0];
        (*state)[1][c] = temp[1] ^ words[round+c][1];
        (*state)[2][c] = temp[2] ^ words[round+c][2];
        (*state)[3][c] = temp[3] ^ words[round+c][3];
    }
}

aes_code_t encrypt(const uint8_t *plaintext, const uint8_t *key, uint8_t *ciphertext) {
    state_t state;
    word expanded_key[44];
    plaintext_to_state(plaintext, &state);
    key_expansion(key, expanded_key);
    
    // Start of AES rounds
    add_round_key(&state, expanded_key, 0);
    for (int round = 1; round <= 9; round++) {
        sub_bytes(&state);
        shift_rows(&state);
        mix_columns(&state);
        add_round_key(&state, expanded_key, round * 4);
    }
    // Final round (no MixColumns)
    sub_bytes(&state);
    shift_rows(&state);
    add_round_key(&state, expanded_key, 40);
    
    state_to_ciphertext(&state, ciphertext);
    return AES_SUCCESS;
}